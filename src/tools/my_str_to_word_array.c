...
static int get_next_idx_delim(
    const char *str, const char *delim, const int begin)
{
    int idx = -1;
...
static int create_words(
    const char *str, const char *delim, char ***ret, const int nb_word)
{
    int prev_idx = 0;
    int idx_delim = 0;
    ...
    for (size_t i = 0; i < (size_t) nb_word; i++) {
        ...
        if (!(*ret)[i])
            return EXIT_FAILURE;
        strncpy((*ret)[i], str + prev_idx, idx_delim - prev_idx);
